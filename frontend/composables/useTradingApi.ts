...
export function useTradingApi() {
  ...
  return {
    ...
    updateWatchlist: (payload: { items: Array<{ symbol: string; category: string; timeframe_seconds: number; enabled: boolean }> }) =>
      request<WatchlistItem[]>('/watchlist', { method: 'PUT', body: payload }),
    ...
    resume: () => request<SystemState>('/system/resume', { method: 'POST' }),
    disconnectBroker: (adminToken: string) => localControl<BrokerConnection>('/broker/disconnect', adminToken, { method: 'POST' }),
    runResearch: (adminToken: string, strategyId?: number) => localControl<ResearchRun>(`/research/run${strategyId != null ? `?strategy_id=${strategyId}` : ''}`, adminToken, { method: 'POST' }),
